...
## Ordenar descendientemente (valores grandes)
...
## Ordenar ascendentemente (valores pequeños)
...
dirty_iris %>% count(Species) # Esto parece que está bien
library(ggplot2)
dirty_iris %>% drop_na() %>% ggplot(aes(x=Species, y= Petal.Length)) + geom_boxplot()
```
...
na_iris <- dirty_iris %>%
  replace(.,.<=0,NA) #%>%
  #mutate_if(is.numeric, across(ifelse(!is.finite(.), NA, .)))
  #replace(.,sapply(., is.finite), NA)

na_iris[-5][!sapply(dirty_iris[,-5], is.finite)] <- NA
...
completas <- sum(complete.cases(na_iris))
print(completas)
p_completas <- sum(complete.cases(na_iris))*100/nrow(na_iris)
print(p_completas)

clean_iris <- na.omit(na_iris)
```
...
```{r , include = TRUE}
clean_iris %>% 
  summarise(across(where(is.numeric), list(Media = mean, `Desviación típica` = sd, Mediana = median, MADM = mad, IQR = IQR ))) #%>%
  #pivot_longer(values_to = starts_with("Sepal.Length"||"Sepal.Width"||"Petal.Length"||"Petal.Width"))
```
...
```{r , include = FALSE}
# RECOMENDACIÓN: Devolver vectores true/false de la misma longitud que el de entrada
# Regla 3 sigma
reglasigma <- function(x){
  res <- x>=(mean(x,na.rm=TRUE)+3*sd(x,na.rm=TRUE))|x<=(mean(x,na.rm=TRUE)-3*sd(x,na.rm=TRUE))
  return(res)
}

reglasigma(na_iris$Petal.Width)

# Identificador Hampel
reglahampel <- function(x){
  res <- x>=(median(x,na.rm=TRUE)+3*mad(x,na.rm=TRUE))|x<=(median(x,na.rm=TRUE)-3*mad(x,na.rm=TRUE))
  return(res)
}

reglahampel(dirty_iris$Petal.Length)
...
reglaboxplot <- function(x){
  ...
  iqr <- x_s-x_i
  res <- x<=(x_i-1.5*iqr)|x>=(x_s+1.5*iqr)
  return(res)
  }

# Percentiles
reglapercentil <- function(x){
  x_s <- quantile(x, probs=0.95, na.rm=T)
  x_i <- quantile(x, probs=0.05, na.rm=T)
  res <- x>x_s|x<x_i
  return(res)
}
```
...
dfoutliers <- clean_iris %>% 
  mutate(`Outliers sigma`=reglasigma(Petal.Length), `Outliers hampel`=reglahampel(Petal.Length), `Outliers boxplot`=reglaboxplot(Petal.Length), `Outliers percentil` = reglapercentil(Petal.Length))

dfoutliersTodos <- clean_iris %>% 
  mutate(across(where(is.numeric), `Outliers sigma`=reglasigma(.), `Outliers hampel`=reglahampel(.), `Outliers boxplot`=reglaboxplot(.), `Outliers percentil` = reglapercentil(.)))
...
num_outliers <- dfoutliers %>% 
  summarise(`Outliers sigma`=sum(`Outliers sigma`), `Outliers hampel`=sum(`Outliers hampel`), `Outliers boxplot`=sum(`Outliers boxplot`), `Outliers percentil` = sum(`Outliers percentil`) )

num_outliers

val_outliers <- dfoutliers %>% 
  select(Petal.Length, starts_with('Outliers')) %>%
  filter(rowSums(starts_with('Outliers'))>=1)
val_outliers
```
